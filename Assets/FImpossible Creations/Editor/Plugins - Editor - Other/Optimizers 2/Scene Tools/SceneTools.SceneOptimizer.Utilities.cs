...
namespace FIMSpace.FOptimizing
{
    public partial class OptimizersPrefabsGrabber
    {
        ...
        List<GameObject> GetAllPrefabsFromScene()
        {
            ...
            try
            {
                // Sửa lỗi: Thay FindObjectsOfType bằng FindObjectsByType với tùy chọn không sắp xếp (tăng tốc độ)
                Transform[] allSceneObjs = FindObjectsByType<Transform>(FindObjectsSortMode.None);
                ...
                foreach (var t in allSceneObjs)
                {
                    ...
                    if (pr != null)
                    {
                        if (!prefabs.Contains(pr))
                        {
                            ...
                            if (optimizer)
                            {
                                // Sửa lỗi: Thay GetInstanceID() bằng GetHashCode()
                                OptimizerSceneData f = AllWithOptimizers.FirstOrDefault(ff => ff.prefabObject.GetHashCode() == pr.GetHashCode());
                                if (f == null) AllWithOptimizers.Add(new OptimizerSceneData(optimizer, pr, scenep));
                            }
                            else
                            {
                                // Sửa lỗi: Thay GetInstanceID() bằng GetHashCode()
                                OptimizerSceneData f = AllWithoutOptimizers.FirstOrDefault(ff => ff.prefabObject.GetHashCode() == pr.GetHashCode());
