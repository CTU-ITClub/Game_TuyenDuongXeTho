...
public class InventorySystem : MonoBehaviourPun // Đổi thành MonoBehaviourPun
{
    ...
    private void Update()
    {
        ...
        {
            Debug.Log($"[InventorySystem] Drop key pressed. Dropping item from slot {EquippedSlotIndex}");
    ...
    public void DropItem(int slotIndex)
    {
        ...
        if (slot.itemData.worldPrefab != null)
        {
            ...
            {
                string prefabPath = slot.itemData.worldPrefab.name;

                photonView.RPC(nameof(RPC_RequestMasterSpawn), RpcTarget.MasterClient, prefabPath, spawnPosition, spawnRotation);
            }
        ...
        // Đồng bộ slot đã xóa cho các client khác
        if (PhotonNetwork.IsConnectedAndReady && PhotonNetwork.InRoom && photonView != null)
        {
            photonView.RPC(nameof(RPC_SyncSlot), RpcTarget.Others, slotIndex, "", 0);
        }
    ...
    [PunRPC]
    private void RPC_RequestMasterSpawn(string prefabPath,  Vector3 pos, Quaternion rot)
    {
        if (!PhotonNetwork.IsMasterClient)
            return;

        Debug.Log($"[InventorySystem] Master spawning: {prefabPath}");

        PhotonNetwork.Instantiate(prefabPath, pos, rot);
    }

    private Vector3 GetDropPosition()
    {
        Vector3 forwardPoint = transform.position + transform.forward * dropDistance + Vector3.up * 2f;
    ...
    public void UseItem(int slotIndex)
    {
        ...
        {
            Debug.Log(
                $"{slot.itemData.itemName} không phải Consumable, không thể Use."
            );
            return;
        }
        ...
        Debug.Log(
            $"Sử dụng {slot.itemData.itemName}, hồi {slot.itemData.healAmount} máu."
        );
        ...
        else
        {
            ...
            if (PhotonNetwork.IsConnectedAndReady && PhotonNetwork.InRoom && photonView != null)
            {
                photonView.RPC(nameof(RPC_SyncSlot), RpcTarget.Others, slotIndex, slot.itemData.name, slot.quantity);
            }
        }
